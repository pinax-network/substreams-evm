...
ALTER TABLE erc20_transfers
    ...
    -- INDEXES --
    ADD INDEX IF NOT EXISTS idx_amount (amount) TYPE minmax,
    ...
    ADD PROJECTION IF NOT EXISTS prj_from_to_by_minute ( SELECT `from`, `to`, minute GROUP BY `from`, `to`, minute ),
    ADD PROJECTION IF NOT EXISTS prj_log_address_by_minute ( SELECT log_address, `from`, minute GROUP BY log_address, `from`, minute ),
    ADD PROJECTION IF NOT EXISTS prj_log_address_to_by_minute ( SELECT log_address, `to`, minute GROUP BY log_address, `to`, minute ),
    ADD PROJECTION IF NOT EXISTS prj_log_address_from_to_by_minute ( SELECT log_address, `from`, `to`, minute GROUP BY log_address, `from`, `to`, minute );
...
ALTER TABLE erc20_approvals
    ...
    ADD PROJECTION IF NOT EXISTS prj_owner_spender_count ( SELECT owner, spender, count(), min(block_num), max(block_num), min(timestamp), max(timestamp), min(minute), max(minute) GROUP BY owner, spender ),
    ...
    ADD PROJECTION IF NOT EXISTS prj_owner_spender_by_minute ( SELECT owner, spender, minute GROUP BY owner, spender, minute );