-- x402 totals grouped for recipient-first access patterns
CREATE TABLE IF NOT EXISTS state_x402_totals_by_recipient (
    ...
    -- projections --
    PROJECTION prj_group_by_recipient (
        SELECT
            min(min_timestamp),
            max(max_timestamp),
            min(min_block_num),
        ...
        GROUP BY recipient, asset, transfer_method, settlement_source, scheme
    )
)
ENGINE = AggregatingMergeTree
ORDER BY (recipient, facilitator, asset, transfer_method, settlement_source, scheme)
SETTINGS deduplicate_merge_projection_mode = 'rebuild'
COMMENT 'x402 totals grouped by recipient first';

CREATE MATERIALIZED VIEW IF NOT EXISTS mv_state_x402_totals_by_recipient
TO state_x402_totals_by_recipient
AS
SELECT
...
FROM x402_payments
GROUP BY recipient, facilitator, asset, transfer_method, settlement_source, scheme;
